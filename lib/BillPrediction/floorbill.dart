// ignore_for_file: prefer_typing_uninitialized_variables, prefer_if_null_operators, unnecessary_null_comparison
...
class FloorBillPred extends StatefulWidget {
  const FloorBillPred({Key? key}) : super(key: key);

  @override
  _FloorBillPredState createState() => _FloorBillPredState();
}

class _FloorBillPredState extends State<FloorBillPred> {
  ...
  totalAmount(String rsValue) {
    ...
    double conversion = double.parse(finalTotalValue.toString());
  ...
  Future flatQueryFunc2(id) async {
    List data = await AllDatabase.instance.getFlatByFId(id);
    List<FlatType> flt = [];
    setState(() {
      flt = data.map((data) => FlatType.fromJson(data)).toList();
  ...
  Future flatQueryFuncForDays(id) async {
    List data = await AllDatabase.instance.getFlatByFId(id);
    List<FlatType> flt = [];
    setState(() {
      flt = data.map((data) => FlatType.fromJson(data)).toList();
    ...
    await roomQueryFuncForDays(flt);
  }

  Future flatQueryFuncForHourEnergy(id) async {
    List data = await AllDatabase.instance.getFlatByFId(id);
    List<FlatType> flt = [];
    setState(() {
      flt = data.map((data) => FlatType.fromJson(data)).toList();
  ...
  Future roomQuery(List<FlatType> flt) async {
    for (int i = 0; i < flt.length; i++) {
      List data = await AllDatabase.instance.getRoomById(flt[i].fltId);
      if (kDebugMode) {
        print("data $data");
      }
  ...
  Future roomQueryFuncForDays(List<FlatType> flt) async {
    var allRoom = List.empty(growable: true);
    for (int i = 0; i < flt.length; i++) {
      List data = await AllDatabase.instance.getRoomById(flt[i].fltId);
      for (int j = 0; j < data.length; j++) {
      ...
      await deviceQueryForDays(allRoom);

      dataMap.putIfAbsent(flt[i].fltName, () => total);
  ...
  Future roomQueryFunc2(List<FlatType> flt) async {
    for (int i = 0; i < flt.length; i++) {
      List data = await AllDatabase.instance.getRoomById(flt[i].fltId);
      for (int j = 0; j < data.length; j++) {
        allRoom.add(data[i]);
      ...
      if (kDebugMode) {
        print("dataMap $_valueMinute");
        print("dataMap $dataMap");
        print("dataMap $varFinalTotalValue");
      }
    }
    // await deviceQuery();
  }

  Future roomQueryFuncForHourEnergy(List<FlatType> flt) async {
    for (int i = 0; i < flt.length; i++) {
      List data = await AllDatabase.instance.getRoomById(flt[i].fltId);
      for (int j = 0; j < data.length; j++) {
        allRoom.add(data[i]);
  ...
  Future deviceQueryForHourEnergy(allRoom) async {
    for (int i = 0; i < allRoom.length; i++) {
      List data = await AllDatabase.instance
          .getDeviceById(allRoom[i]['r_id'].toString());
      if (data.isEmpty) {
  ...
  Future deviceQueryForDays(allRoom) async {
    for (int i = 0; i < allRoom.length; i++) {
      List data = await AllDatabase.instance
          .getDeviceById(allRoom[i]['r_id'].toString());
      if (data.isEmpty) {}
  ...
  allFindFunc() async {
    ...
    await flatQueryFuncForDays(floorId);
  }
