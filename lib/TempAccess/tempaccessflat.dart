...
class TempAccessFlat extends StatefulWidget {
  var flatId;
  var ownerName;
  TempAccessFlat({Key? key, required this.flatId, required this.ownerName})
      : super(key: key);

  @override
  _TempAccessFlatState createState() => _TempAccessFlatState();
}

class _TempAccessFlatState extends State<TempAccessFlat>
  ...
  getAllScrollFunc() async {
    await getPinStatusData(deviceIdForScroll);
    await getPinNames(deviceIdForScroll,0);
  }
                                                                  ...
                                                                  Column(
                                                                      children: <
                                                                        ...
                                                                        Row(
                                                                          children: <
                                                                              Widget>[
                                                                            Text(sensorData!.sensor4.toString(),
                                                                                style: const TextStyle(fontSize: 14, color: Colors.white70)),
                                                                          ],
                                                                        ),
                                                                      ]),
  ...
  Future<bool> getFlatName() async {
    String? token = await getToken();
    String url = api + "getyouflatname/?flt_id=" + widget.flatId;
    ...
    if (response.statusCode == 200) {
      flat = jsonDecode(response.body);
      await getRoomForTempUser();
      return true;
    } else {
      if (kDebugMode) {
        print("ans ${response.statusCode}");
      }
      return true;
    }
  }
  ...
  Future getRoomForTempUser() async {
    String? token = await getToken();
    final url = api +
        'getallroomsbyonlyflooridf_id/?flt_id=' +
  ...
  Future<bool> getSensorData(dId) async {
    String? token = await getToken();
    var url = api + 'tensensorsdata/?d_id=' + dId;
    final response = await http.get(Uri.parse(url), headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Token $token',
